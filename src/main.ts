...
class KeyMap implements Iterable<KeyPress> {
	...
	public repr(): string {
		return this.commandID + ' = ' + this.sequence.map( key => key.repr() ).join( ' => ' );
	}
}
...
class RegisterMachine implements StateMachine<KeyPress, KeyRegisterState> {
		...
		switch ( this.currentState ) {
			case KeyRegisterState.NoKeys: {
				if ( !event.containsKey() ) {
					this.currentSequence.pop();
					return KeyRegisterState.NoKeys;
				}
			...
			case KeyRegisterState.AddedKeys: {
				if ( !event.containsKey() ) {
					...
					return KeyRegisterState.RetainedKeys;
				}
	...
	public representation() : string {
		...
		return this.presses().map( key => key.repr()).join(' => ')
	}
	private reset(): void {
		this.currentState    = KeyRegisterState.NoKeys;
		this.currentSequence = [];
	...
	private isSpecial( event: KeyPress ): boolean {
		return (
			event.key === 'Enter'
		);
	}
...
class SetHotkeyModal extends Modal {
	...
	private readonly handleKeyDown = ( event: KeyboardEvent ): void => {
		...
		switch ( registerState ) {
			...
			case KeyRegisterState.RetainedKeys:
				writeConsole('An keypress resulted in a RetainedKeys state. Awaiting further keypresses.')
				event.preventDefault()
				return

			case KeyRegisterState.FirstKey:
				event.preventDefault()
				writeConsole('An keypress resulted in a FirstKey state. Awaiting further keypresses.')
				this.setText(  this.keyRegister.representation() )
				return
			
			case KeyRegisterState.BacktrackedKey:
				event.preventDefault()
				writeConsole('An keypress resulted in a BacktrackedKey state. Awaiting further keypresses.')
				this.setText(  this.keyRegister.representation() )
				return
			
			case KeyRegisterState.AddedKeys:
				event.preventDefault()
				writeConsole('An keypress resulted in a AddedKeys state. Awaiting further keypresses.')
				this.setText(  this.keyRegister.representation() )
				return

			case KeyRegisterState.PendingConfirmation:
				event.preventDefault()
				writeConsole('An keypress resulted in a PendingConfirmation state. Showing confirmation text.')
				...
				confirmText.setText( 'Did you mean literal "lastKey"?. If so, Press Enter.' +
			...
			case KeyRegisterState.FinishedRegistering:
				event.preventDefault()
				writeConsole('An keypress resulted in a FinishedRegistering state. Setting new key.')
