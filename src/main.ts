...
class RegisterMachine implements StateMachine<KeyPress, KeyRegisterState> {
  ...
  public advance(event: KeyPress): KeyRegisterState {
    ...
    switch (this.currentState) {
      ...
      case KeyRegisterState.PendingConfirmation: {
        ...
        this.currentSequence.pop()
        const lastKey = this.currentSequence.pop()
        {
          if ( event.key === 'Enter' ) {
            this.currentSequence.push( lastKey )
            this.currentState = KeyRegisterState.AddedKeys;
  ...
  private isSpecial(event: KeyPress): boolean {
    return (
        event.key === 'Enter' || event.key === 'Backspace' || event.key === 'Escape'
    );
  }
...
class SetHotkeyModal extends Modal {
  ...
  public onOpen = (): void => {
    this.
    const introText =
    introText.setText('Press a key to use as the hotkey.');
